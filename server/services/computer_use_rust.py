...
- get_a11y_tree / generate_som_overlay / ocr_recognize: 调用时若 HAS_RUST,
  走真实 Rust 调用路径;否则返回 mock 数据,字段结构与未来 Rust 返回值保持一致
- get_status: 仅查询并汇报 HAS_RUST 状态(无 Rust/mock 数据分支),
  返回 rust_available/rust_version/skeleton/mock 字段
...
- (无 Rust 对应): ComputerUseRust.get_status — Python 状态查询
"""
